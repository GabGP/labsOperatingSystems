...
reset_handler:
    ...
    ldr sp, =_stack_top     @ Use the top of our stack for IRQ

    @ Set up System/User mode stack
    msr cpsr_c, #0xDF       @ Switch to System mode
    ldr sp, =(_stack_top - 0x1000) @ Give IRQ mode 4KB of space

    @ Set up exception vector table base address (VBAR)
    ldr r0, =vector_table
...
irq_handler:
    ...
    ldmfd sp!, {r0-r12, pc}^ @ Restore and return (caret restores CPSR)
